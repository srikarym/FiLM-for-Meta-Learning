...
class Model(object):
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        filmObj = filmInit(nenvs)
        act_model = policy(sess, ob_space, ac_space, nbatch_act, 1, filmObj, reuse=False)
        train_model = policy(sess, ob_space, ac_space, nbatch_train, nsteps, filmObj, reuse=True)
...
class Runner(AbstractEnvRunner):
    ...
    def run(self, update):
        ...
        for s in range(self.nsteps):
            actions, values, self.states, neglogpacs = self.model.step(self.obs, nenvs, self.states, self.dones)
        ...
        last_values = self.model.value(self.obs, nenvs, self.states, self.dones)
...
def learn(*, policy, env, nsteps, total_timesteps, ent_coef, lr,
            ...
            save_interval=0, load_path=None, log_path = ''):
    ...
    make_model = lambda : Model(policy=policy, ob_space=ob_space, ac_space=ac_space, nbatch_act=num_envs, nbatch_train=nbatch_train,
    ...
    for update in range(1, nupdates+1):
        ...
        if states is None: # nonrecurrent version
            ...
            assert num_envs % nminibatches == 0
            envsperbatch = num_envs // nminibatches
            envinds = np.arange(num_envs)
            flatinds = np.arange(num_envs * nsteps).reshape(num_envs, nsteps)
            ...
            for _ in range(noptepochs):
                ...
                for start in range(0, num_envs, envsperbatch):
                    ...
                    mblossvals.append(model.train(nenvs,lrnow, cliprangenow, *slices, mbstates))
